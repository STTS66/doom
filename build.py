...
<body>
    <div id="startMenu">
        ...
        <div id="lobbyPanel">
            ...
            <div id="playerListBox">
                <div style="color: #666; font-size: 12px; margin-bottom: 8px;">ИГРОКИ В КОМНАТЕ:</div>
                <div id="connectedPlayersList">Подключение к серверу...</div>
            </div>
            
            <div id="inviteBox" style="display: none;">
                <p style="color: #aaa; font-size: 14px; margin-bottom: 10px;" id="inviteTextDesc">Пригласите друга по этой ссылке:</p>
            ...
            <button class="authBtn" id="startGameBtn" style="background: #0a0; border-color: #0f0; font-size: 20px; margin-top: 15px;">🎮 ЗАЙТИ В АРЕНУ</button>
    ...
    // Генерируем URL грамотно
    const urlParams = new URLSearchParams(window.location.search);
    if(urlParams.get('room')) {
        targetRoomID = urlParams.get('room');
        isGuest = true;
    } else {
        targetRoomID = 'ROOM_' + Math.random().toString(36).substring(2, 7).toUpperCase();
        isGuest = false;
    }
    ...
    function showLobby() {
        ...
        // ПОКАЗЫВАТЬ ссылку ТОЛЬКО тому, кто ее создал
        if (!isGuest) {
            document.getElementById('inviteBox').style.display = 'block';
            let pureURL = window.location.origin + window.location.pathname;
            document.getElementById('inviteLink').innerText = pureURL + "?room=" + targetRoomID;
        } else {
            document.getElementById('inviteBox').style.display = 'none';
        }

        initMultiplayer(); // Подключаемся к комнате Photon
    }
    ...
    document.getElementById('startGameBtn').addEventListener('click', () => {
        if(photon && photon.myRoom()) {
            ...
            alert('Сначала дождитесь успешного подключения к серверу лобби!');
        }
    });

    // === ПРАВИЛЬНЫЙ КОД PHOTON SDK v4.1 ===
    function initMultiplayer() {
        ...
        photon.addEventListener(Photon.LoadBalancing.EventName.CONNECTED_TO_MASTER, () => {
            ...
            console.log("Connected to Master, joining room: " + targetRoomID);
        ...
        photon.addEventListener(Photon.LoadBalancing.EventName.ACTOR_JOINED, (actor) => {
            console.log("Actor joined: ", actor.name);
            updatePlayerListUI();
        });

        photon.addEventListener(Photon.LoadBalancing.EventName.ACTOR_LEFT, (actor) => {
            if(otherPlayers.has(actor.actorNr)) { 
                scene.remove(otherPlayers.get(actor.actorNr)); 
                otherPlayers.delete(actor.actorNr); 
            }
            updatePlayerListUI();
        });

        photon.addEventListener(Photon.LoadBalancing.EventName.RAISE_EVENT, (code, content, actorNr) => {
            if(code === 1 && gameStarted) {
                ...
                p.position.set(content.x, content.y, content.z); p.rotation.y = content.ry;
            }
        });
    }
    ...
    function updatePlayerListUI() {
        ...
        let names = [];
        photon.actors.getAll().forEach(act => {
            const isMe = (act.actorNr === photon.myActor().actorNr) ? " (Вы)" : "";
            names.push('<div class="player-item">👤 ' + (act.name || "Игрок") + isMe + '</div>');
        });
    ...
    function initScene() {
        ...
        scene.add(new THREE.AmbientLight(0xffffff, 0.8)); // Освещение
        ...
        // Если кто-то уже есть в комнате, надо его отобразить на сцене сразу
        if(photon && photon.myRoom()) {
            photon.actors.getAll().forEach(act => {
                if (act.actorNr !== photon.myActor().actorNr) {
                    if(!otherPlayers.has(act.actorNr)) {
                        const op = new THREE.Mesh(new THREE.BoxGeometry(1.2, 2.4, 1.2), new THREE.MeshStandardMaterial({ color: 0x00aaff }));
                        scene.add(op); otherPlayers.set(act.actorNr, op);
                    }
                }
            });
        }
    ...
    function animate() {
        ...
        if(document.pointerLockElement) {
            ...
            if (move.length() > 0) camera.position.add(move.normalize().multiplyScalar(0.20)); // Чуть быстрее ход
